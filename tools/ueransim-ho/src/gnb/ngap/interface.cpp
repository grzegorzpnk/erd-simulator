...
//kasia
void NgapTask::handleN2Handover(int ueId, int amf_ue_ngap_id, long ran_ue_ngap_id)
{
    ...
    sendNgHandoverRequired(ueId, amf_ue_ngap_id, ran_ue_ngap_id, nr::gnb::HandoverType::Intra5GS, 
                                nr::gnb::NgapCause::RadioNetwork_handover_desirable_for_radio_reason, 2);
                                ...
                                NgapCause cause, int global_ran_node_id)
{
    ...
    asn::SetOctetString3(globalGnbId->pLMNIdentity, ngap_utils::PlmnToOctet3(m_base->config->plmn));
    globalGnbId->gNB_ID.present = ASN_NGAP_GNB_ID_PR_gNB_ID;
    asn::SetBitString(globalGnbId->gNB_ID.choice.gNB_ID, octet4{17185}, //hardcoded, based on gNB config
                      static_cast<size_t>(m_base->config->gnbIdLength));
...
{
    m_logger->debug("Sending Handover Requiest Acknowledge");
