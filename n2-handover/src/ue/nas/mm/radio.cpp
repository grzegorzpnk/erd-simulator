...
m_logger->debug("handle Active Cell Change");
...
//kasia
void NasMm::handleRrcReconfiguration(){

    m_logger->debug("Radio link reconfiguration");
    switchMmState(EMmSubState::MM_REGISTERED_INITIATED_PS);
    //switchCmState(ECmState::CM_CONNECTED);
    /*switchCmState(ECmState::CM_IDLE);
    if (m_mmState == EMmState::MM_REGISTERED)
        switchMmState(EMmSubState::MM_REGISTERED_PS);*/
    // UeRrcTask::changeActiveCell();
/*
    handleRrcConnectionRelease();
