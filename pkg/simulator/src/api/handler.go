...
func (h *apiHandler) generateICCTunningHeuristicChart(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	err := h.ResultClient.GenerateChartPkgAppsICCTunning(results.RelocationTriggeringRates, basePath+"/"+apps)
	if err != nil {
		log.Errorf("Error: %v", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	...
	err = h.ResultClient.GenerateChartPkgAppsICCTunning(results.RelocationRejectionRates, basePath+"/"+apps)
	if err != nil {
		log.Errorf("Error: %v", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
	...
	err = h.ResultClient.GenerateChartPkgMecsICCTunning(results.ResCpu, basePath+"/"+mecs)
	if err != nil {
		log.Errorf("Error: %v", err)
		w.WriteHeader(http.StatusInternalServerError)
		return
...
func (h *apiHandler) conductExperimentGlobcom(w http.ResponseWriter, r *http.Request) {
	...
	//log.Infof("Started new full GLOBECOM (ML) experiment with all %v types: Optimal, EAR, RL-masked, RL-no-masked", len(experiments))
	log.Infof("Started new full GLOBECOM (ML) experiment with all %v types.", len(experiments))
