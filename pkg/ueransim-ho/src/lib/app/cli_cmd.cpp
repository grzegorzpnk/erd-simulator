...
static OrderedMap<std::string, CmdEntry> g_gnbCmdEntries = {
    ...
    {"handover", {"Performs N2 handover", "<ue-id> <target-cell-id>", DefaultDesc, false}},
};
                                                      ...
                                                      std::string &error)
{
    ...
    {
        auto cmd = std::make_unique<GnbCliCommand>(GnbCliCommand::UE_HANDOVER);
        if (options.positionalCount() == 0)
            CMD_ERR("UE ID is expected")
        cmd->ueId = utils::ParseInt(options.getPositional(0));
        if (cmd->ueId <= 0)
            CMD_ERR("Invalid UE ID")
        cmd->targetCellId = utils::ParseInt(options.getPositional(1));
        if (cmd->targetCellId <= 0)
            CMD_ERR("Invalid Target Cell Id")
        return cmd;
    }
