...
void GnbCmdHandler::handleCmdImpl(NmGnbCliCommand &msg)
{
    ...
    case app::GnbCliCommand::UE_HANDOVER: {
        ...
        else
        {
            auto ue = m_base->ngapTask->m_ueCtx[msg.cmd->ueId];
                json.push(Json::Obj({
                {"ue-id", msg.cmd->ueId},
                {"ran-ngap-id", ue->ranUeNgapId},
                {"amf-ngap-id", ue->amfUeNgapId},
            }));
                    
            m_base->ngapTask->handleN2Handover(msg.cmd->ueId, ue->amfUeNgapId, ue->ranUeNgapId, msg.cmd->targetCellId); //pmq
            hoTargetCellId = msg.cmd->targetCellId; //pmq

            sendResult(msg.address,"Handle N2 Handover for: \n" + json.dumpYaml());
        }
        break;
