    ...
    case rrc::RrcChannel::DL_DCCH: {
        if (isActiveCell(cellId))
        {
            ...
            if (pdu == nullptr)
                ...
                if (hoTargetCellId <= 0)
                    m_logger->err("global variable: hoTargetCellId is not correct!");
                receiveRrcMessage(hoTargetCellId, pdu);
...
void UeRrcTask::receiveRrcMessage(int cellId, ASN_RRC_DL_DCCH_Message *msg)
{
    ...
    case ASN_RRC_DL_DCCH_MessageType__c1_PR_rrcReconfiguration:
        receiveRrcReconfiguration(*c1->choice.rrcReconfiguration, cellId);
        break;
    default:
        break;
    }
