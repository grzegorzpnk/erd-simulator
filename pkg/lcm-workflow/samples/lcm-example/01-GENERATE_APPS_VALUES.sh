...
#  echo "$(expr $((10 + $RANDOM % 30)))"
# need different apps [16: 30ms], [17, 15ms], [17, 10ms]
  mod=$(expr $1 % 3)
  if [[ $mod == 0 ]]; then
    echo 30
  elif [[ $mod == 1 ]]; then
...
#    index=$(expr $(($RANDOM % (${#clusters[@]}))))
    cpu_req=$(expr $((500 + $RANDOM % 500)))
    mem_req=$(expr $((500 + $RANDOM % 500)))
    ...
    for var in "${vars[@]}"; do
      ...
      if [[ $var == "TARGET_CLUSTER_" ]]; then
        ...
        export "$temp"="$(generateMaxLatency $app)"
